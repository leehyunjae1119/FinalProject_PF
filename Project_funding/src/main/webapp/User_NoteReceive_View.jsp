...
<link href="resources/assets/css/User_NoteReceive_View.css"
	rel="stylesheet">
</head>
...
<!-- 메세지 페이징 제이쿼리 -->
<script type="text/javascript" src="resources/Message_Paging/pasing.js?ver=1"></script>
				...
				<div class="row">
					...
					<div class="col" style="margin-top: -30px;">
						...
						<div class="col col-md-15">
							...
							<form action="muldel.do?message_no=${MessageView_message_no }"
								method="post">
								<c:if test="${MessageView.message_state eq '안읽음' }">
								...
								<table class="table table table-hover"
									...
									<tbody>
										<c:choose>
											<c:when test="${empty reMessage }">
												<tr>
													<td colspan="4" align="center">===== 받은 쪽지가 없습니다. =====</td>
												</tr>
											</c:when>
											<c:otherwise>
												<c:forEach items="${reMessage }" var="dto">
													<c:if test="${dto.message_reader eq userdto.user_id }">
														<tr>
															<td><input type="checkbox" name="chk" value="message_no=${MessageView.message_no }">&nbsp;</td>

															<!-- 읽음 / 안읽음 상태 표시 조건 -->
															<c:if test="${dto.message_state eq '안읽음' }">
																<td ><b><a href="message_view.do?message_no=${dto.message_no }"
																	class="message" id="mcontent">${dto.message_content }</a></b></td>
															</c:if>
															<c:if test="${dto.message_state eq '읽음' }">
																<td><a href="message_view.do?message_no=${dto.message_no }" class="message" id="mcontent">${dto.message_content }</a></td>
															</c:if>
